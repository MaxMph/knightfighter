...
func _process(delta: float) -> void:
	$wind.volume_db = -55 + clamp(get_parent().linear_velocity.length() * 1.3, 0, 50)
	#$wind.volume_db = move_toward($wind.volume_db, -60 + clamp(get_parent().linear_velocity.length() * 1.2, 0, 60) , delta * 80)
	
	if (abs(get_parent().linear_velocity.x) + abs(get_parent().linear_velocity.z)) > 3 and $"../floorcast".is_colliding():
